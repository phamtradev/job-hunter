...
public class GlobalException {
    ...
    public ResponseEntity<RestRespone<Object>> handleHttpMessageNotReadable(HttpMessageNotReadableException ex) {
        RestRespone<Object> res = new RestRespone<>();
        res.setStatusCode(HttpStatus.BAD_REQUEST.value());
        res.setError("Request body không hợp lệ");
        
        String message = ex.getMessage();
        if (message != null && message.contains("JSON")) {
            res.setMessage("Định dạng JSON không hợp lệ. Vui lòng kiểm tra lại request body.");
        } else {
            res.setMessage("Request body không đúng định dạng hoặc thiếu thông tin bắt buộc.");
        }
